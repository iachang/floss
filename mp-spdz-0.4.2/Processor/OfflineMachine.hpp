...
OfflineMachine<W>::OfflineMachine(int argc, const char **argv,
    ...
    : W(argc, argv, opt, online_opts, V(), nplayers),
      playerNames(W::playerNames), P(*this->new_player("machine")) {
...
template <class W> OfflineMachine<W>::~OfflineMachine() { delete &P; }

template <class W> template <class T, class U> int OfflineMachine<W>::run() {
  ...
  Machine<T, U>::init_binary_domains(this->online_opts.security_parameter,
                                     this->get_lg2());
  auto binary_mac_key =
      read_generate_write_mac_key<typename T::bit_type::part_type>(P);
  ...
  // Get communication statistics and print benchmarks
  NamedCommStats comm_stats = P.total_comm();
  print_timers();
  if (T::is_real)
    print_comm(P, comm_stats);
...
template <class W> template <class T> void OfflineMachine<W>::generate() {
  ...
  for (unsigned i = 0; i < domain_usage.size(); i++) {
    ...
    Dtype dtype = Dtype(i);
    string filename = Sub_Data_Files<T>::get_filename(playerNames, dtype, 0);
    if (my_usage > 0) {
      ofstream out(filename, iostream::out | iostream::binary);
      file_signature<T>().output(out);
      if (i == DATA_DABIT) {
        for (long long j = 0; j < buffered_total(my_usage, BUFFER_SIZE); j++) {
          T a;
  ...
  for (int i = 0; i < P.num_players(); i++) {
    auto n_inputs = usage.inputs[i][T::clear::field_type()] + additional_inputs;
    string filename = Sub_Data_Files<T>::get_input_filename(playerNames, i, 0);
    if (n_inputs > 0) {
      ofstream out(filename, iostream::out | iostream::binary);
      file_signature<T>().output(out);
      InputTuple<T> tuple;
      for (long long j = 0; j < buffered_total(n_inputs, BUFFER_SIZE); j++) {
  ...
  if (T::clear::field_type() == DATA_INT) {
    ...
    for (int n_bits = 1; n_bits < max(100, max_n_bits); n_bits++) {
      ...
      int total =
          usage.edabits[{false, n_bits}] + usage.edabits[{true, n_bits}];
      string filename =
          Sub_Data_Files<T>::get_edabit_filename(playerNames, n_bits, 0);
      if (total > 0) {
        ...
        for (int i = 0; i < buffered_total(total, batch) / batch; i++)
          preprocessing.get_edabitvec(true, n_bits).output(n_bits, out);
      } else
        remove(filename.c_str());
    }
  }
...
template <class W> const Names &OfflineMachine<W>::get_N() {
  return playerNames;
}
