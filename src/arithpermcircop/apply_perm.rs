...
use crate::{
    ...
    utils::rng_utils::{get_random_permutation_usize, shuffle_vector_testing},
};

/// ApplyPerm(([perm_share]_0, [x]_0), ([perm_share]_1, [x]_1))) -> perm(x)
pub struct ApplyPerm<F: Field> {
    _phantom: std::marker::PhantomData<F>,
}

impl<F: Field> ApplyPerm<F> {
    ...
    pub fn run_with_perm_network_timing(
        ...
        let p0_inverse_perm_share = if net.party_id() == 0 {
            ...
            PermNetworkShuffle::<F>::run(net, state, (None, perm_auth_share.clone(), true))
        };

        // Apply P1 random shuffle inverse to the P0 inverse perm share
        let p1p0_inverse_shuffle_share = if net.party_id() == 0 {
            PermNetworkShuffle::<F>::run_with_timing(
                net,
                state,
                (None, p0_inverse_perm_share.clone(), true),
                &mut timing_callback,
            )
        } else {
        ...
        let p1p0_inverse_shuffle =
            VectorReveal::<F>::run(net, state, p1p0_inverse_shuffle_share.clone());

        // Apply the randomized composed perm shuffle to the input auth shares
        let p1p0_inverse_shuffle_output =
            shuffle_vector_testing(&p1p0_inverse_shuffle, &input_auth_share);

        // Undo P1 random shuffle inverse
        let p0_inverse_shuffle_output = if net.party_id() == 0 {
            PermNetworkShuffle::<F>::run_with_timing(
                net,
                state,
                (None, p1p0_inverse_shuffle_output.clone(), false),
                &mut timing_callback,
            )
        } else {
            PermNetworkShuffle::<F>::run(
                ...
                (
                    Some(p1_random_perm_for_network.clone()),
                    p1p0_inverse_shuffle_output.clone(),
                    false,
                ),
            )
        };

        // Undo P0 random shuffle inverse
        let apply_perm_output = if net.party_id() == 0 {
            PermNetworkShuffle::<F>::run_with_timing(
                net,
                state,
                (
                    Some(p0_random_perm_for_network.clone()),
                    p0_inverse_shuffle_output.clone(),
                    false,
                ),
                &mut timing_callback,
            )
            ...
            PermNetworkShuffle::<F>::run(
                net,
                state,
                (None, p0_inverse_shuffle_output.clone(), false),
            )
        };
    ...
    /// Run the apply perm operation with floss with timing callback
    pub fn run_with_floss_timing(
        ...
        let p0_inverse_perm_share = Shuffle::<F>::run(
            net,
            state,
            (
                0,
                p0_random_shuffle_id.clone() + "_inverse",
                perm_auth_share.clone(),
            ),
        );
        let p1p0_inverse_shuffle_share = Shuffle::<F>::run(
            net,
            state,
            (
                1,
                p1_random_shuffle_id.clone() + "_inverse",
                p0_inverse_perm_share.clone(),
            ),
        );
        ...
        let p0_inverse_shuffle_output = Shuffle::<F>::run(
            net,
            state,
            (
                1,
                p1_random_shuffle_id.clone(),
                p1p0_inverse_shuffle_output.clone(),
            ),
        );

        // Undo P0 random shuffle inverse
        let apply_perm_output = Shuffle::<F>::run(
            net,
            state,
            (
                0,
                p0_random_shuffle_id.clone(),
                p0_inverse_shuffle_output.clone(),
            ),
        );
    ...
    /// Run the apply perm operation with simple perm network
    pub fn run_with_simple_perm_network_timing(
        ...
        let p0_inverse_perm_share = if net.party_id() == 0 {
            ...
            SimplePermNetShuffle::<F>::run(net, state, (None, perm_auth_share.clone(), true))
        };
        timing_callback(shuffle_time_0.elapsed());
        ...
        let shuffle_time_1 = Instant::now();
        let p1p0_inverse_shuffle_share = if net.party_id() == 0 {
            SimplePermNetShuffle::<F>::run(net, state, (None, p0_inverse_perm_share.clone(), true))
        } else {
            SimplePermNetShuffle::<F>::run(
                net,
                state,
...
impl<F: Field> ArithPermCircOp<F> for ApplyPerm<F> {
    type In = (
        String,            // p0_random_shuffle_id
        String,            // p1_random_shuffle_id
        Vec<AuthShare<F>>, // shuffle share
    ...
    // Requires one regular shuffle tuple, one inverse shuffle tuple.
    fn run(net: &mut Net, state: &mut ArithPermCircState<F>, input: Self::In) -> Self::Out {
        ...
        let p0_inverse_perm_share = if with_perm_network {
            if net.party_id() == 0 {
                PermNetworkShuffle::<F>::run(
                    net,
                    state,
        ...
        // Apply P1 random shuffle inverse to the P0 inverse perm share
        let p1p0_inverse_shuffle_share = if with_perm_network {
            if net.party_id() == 0 {
                PermNetworkShuffle::<F>::run(
                    net,
                    state,
        ...
        // Undo P1 random shuffle inverse
        let p0_inverse_shuffle_output = if with_perm_network {
            if net.party_id() == 0 {
                PermNetworkShuffle::<F>::run(
                    net,
                    state,
        ...
        // Undo P0 random shuffle inverse
        let apply_perm_output = if with_perm_network {
            if net.party_id() == 0 {
                PermNetworkShuffle::<F>::run(
                    net,
                    state,
...
mod test {
    ...
    use crate::utils::rng_utils::{get_random_permutation, get_random_permutation_usize};
    ...
    fn test_apply_perm() {
        ...
        let random_shuffle_p0_clone = random_shuffle_p0.clone();
        let random_shuffle_p1 = get_random_permutation_usize(n);
        let random_shuffle_p1_clone = random_shuffle_p1.clone();

        let random_shuffle_to_share = get_random_permutation::<Fr>(n);
        ...
        rayon::scope(|s| {
            ...
            s.spawn(move |_| {
                ...
                let mut arith_perm_circ_state = arith_perm_circ_prep.run(
                    ...
                    vec![
                        ShuffleTupleInput {
                            ...
                            shuffle: Some(random_shuffle_p0_clone.clone()),
                            n: n,
                ...
                let party0_apply_perm_output = ApplyPerm::<Fr>::run(
                    &mut net,
                ...
                let apply_perm_output = VectorReveal::<Fr>::run(
                    &mut net,
                    &mut arith_perm_circ_state,
                ...
                outputs_clone
                    .lock()
                    .unwrap()
                    .push((party0_auth_shares, apply_perm_output));
            });
            ...
            s.spawn(move |_| {
                ...
                let mut arith_perm_circ_state = arith_perm_circ_prep.run(
                    ...
                    vec![
                        ...
                        ShuffleTupleInput {
                            shuffle_id: "random_shuffle_p1".to_string(),
                            shuffle: Some(random_shuffle_p1_clone.clone()),
                            n: n,
                            num_shuffle_tuples: 1,
                            with_inverse: true,
                        },
                ...
                let party1_apply_perm_output = ApplyPerm::<Fr>::run(
                    &mut net,
                    &mut arith_perm_circ_state,
                    (
                        "random_shuffle_p0".to_string(),
                ...
                outputs_clone
                    .lock()
                    .unwrap()
                    .push((party1_auth_shares, apply_perm_output));
            });
        });
        ...
        if combined_outputs.len() == 2 {
            let (party0_auth_shares, _) = combined_outputs[0].clone();
            let (party1_auth_shares, apply_perm_output) = combined_outputs[1].clone();
            let party0_shuffled_auth_shares = shuffle_vector_testing::<Fr, AuthShare<Fr>>(
                &random_shuffle_to_share_clone.clone(),
                &party0_auth_shares,
            );
            let party1_shuffled_auth_shares = shuffle_vector_testing::<Fr, AuthShare<Fr>>(
                &random_shuffle_to_share_clone.clone(),
                &party1_auth_shares,
            );
    ...
    #[test]
    fn test_apply_perm_with_perm_network() {
        let n = 25;
    ...
    #[test]
    fn test_apply_perm_with_simple_perm_network() {
        let n = 25;
        ...
        rayon::scope(|s| {
            ...
            s.spawn(move |_| {
                ...
                // Generate arithmetic permutation circuit state with shuffle tuples
                let mut shuffle_time_duration_apply_perm = std::time::Duration::ZERO;
                let mut arith_perm_circ_prep = DummyArithPermCircPrep::<Fr>::new();
                let mut arith_perm_circ_state =
                    arith_perm_circ_prep.run(&mut net, &mut state, vec![]);
                let party0_apply_perm_output = ApplyPerm::<Fr>::run_with_simple_perm_network_timing(
                    &mut net,
                    &mut arith_perm_circ_state,
                    (random_shuffle_shares_p0.clone(), party0_auth_shares.clone()),
                    &mut |shuffle_time| {
                        shuffle_time_duration_apply_perm += shuffle_time;
                    },
                );
            ...
            s.spawn(move |_| {
                ...
                let mut shuffle_time_duration_apply_perm = std::time::Duration::ZERO;
                let party1_apply_perm_output = ApplyPerm::<Fr>::run_with_simple_perm_network_timing(
                    &mut net,
                    &mut arith_perm_circ_state,
